      ...
      <div className="madrid-container py-2 md:py-4 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-2 md:gap-3" onClick={closeMenu}>
          ...
          <span className="font-montserrat font-bold text-sm sm:text-lg md:text-xl lg:text-2xl tracking-tight">
            {siteName.includes(' ') ? (
              <>
                <span className="text-madrid-blue">{siteName.split(' ')[0]}</span>
                <span className="text-black dark:text-white">{siteName.split(' ').slice(1, -1).join(' ')}</span>
                <span className="text-madrid-gold">{siteName.split(' ').slice(-1)[0]}</span>
              </>
            ) : (
              <span className="text-madrid-blue">{siteName}</span>
            )}
          </span>
        </Link>
