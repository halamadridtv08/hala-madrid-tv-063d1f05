...
export const TacticalFormation = ({
  ...
  const PlayerOnPitch = ({ 
    ...
    const badgeBg = isOpposing ? 'bg-red-600' : 'bg-white';
    const badgeText = isOpposing ? 'text-white' : 'text-blue-600';
        ...
        <div className="relative">
          ...
          {player.player_image_url ? (
            <img
              ...
              className={`w-10 h-10 md:w-12 md:h-12 rounded-full border-2 ${borderColor} shadow-lg object-cover bg-gray-800`}
            />
          ) : (
            <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full border-2 ${borderColor} bg-gradient-to-br ${bgGradient} shadow-lg flex items-center justify-center`}>
              <span className="text-white text-xs font-bold">{player.jersey_number}</span>
            </div>
          )}

          {/* Numéro du maillot */}
          <div className={`absolute -top-1 -right-1 ${badgeBg} ${badgeText} text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-md text-[10px]`}>
          ...
          <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-black/80 text-white text-[10px] px-1.5 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-20">
            {player.player_name}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-3 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
        ...
        {player.player_image_url ? (
          <img
            src={player.player_image_url}
            alt={player.player_name}
            className={`w-10 h-10 rounded-full border-2 ${isOpposing ? 'border-red-400' : 'border-blue-400'} object-cover bg-gray-700`}
          />
        ) : (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            ...
            <div className="flex items-center gap-0.5">
              {goals.map((_, idx) => (
                <svg key={`g-${idx}`} className="w-3 h-3" viewBox="0 0 24 24">
                  <circle cx="12" cy="12" r="10" fill="#1a1a1a" stroke="#333" strokeWidth="1"/>
                  <path d="M12 2 L14 8 L20 8 L15 12 L17 19 L12 15 L7 19 L9 12 L4 8 L10 8 Z" fill="white"/>
                </svg>
              ))}
              {assists.map((_, idx) => (
                <div key={`a-${idx}`} className="w-3 h-3 bg-green-500 rounded-full flex items-center justify-center">
              ...
              {cards.map((card, idx) => (
                <div 
                  key={`c-${idx}`} 
                  className={`w-2 h-3 rounded-sm ${card.entry_type === 'red_card' ? 'bg-red-600' : 'bg-yellow-400'}`}
                />
              ))}
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {opposingFormation && (
              <Badge variant="outline" className="bg-red-600/10 border-red-600">
        ...
        <CardContent>
          ...
          >
            ...
            <div className="absolute inset-0">
              <div className="absolute inset-2 border-2 border-white opacity-80 rounded" />
              <div className="absolute w-full h-0.5 bg-white top-1/2 transform -translate-y-0.5 opacity-80" />
              <div className="absolute w-24 h-24 border-2 border-white rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-80" />
              <div className="absolute w-2 h-2 bg-white rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
              <div className="absolute w-2/5 h-[12%] border-2 border-white border-t-0 top-2 left-1/2 transform -translate-x-1/2 opacity-80" />
              <div className="absolute w-1/5 h-[6%] border-2 border-white border-t-0 top-2 left-1/2 transform -translate-x-1/2 opacity-80" />
              <div className="absolute w-2/5 h-[12%] border-2 border-white border-b-0 bottom-2 left-1/2 transform -translate-x-1/2 opacity-80" />
              <div className="absolute w-1/5 h-[6%] border-2 border-white border-b-0 bottom-2 left-1/2 transform -translate-x-1/2 opacity-80" />
              <div className="absolute w-2 h-2 bg-white rounded-full top-[15%] left-1/2 transform -translate-x-1/2" />
              <div className="absolute w-2 h-2 bg-white rounded-full bottom-[15%] left-1/2 transform -translate-x-1/2" />
              <div className="absolute w-16 h-8 border-2 border-white border-t-0 top-[12%] left-1/2 transform -translate-x-1/2 rounded-b-full opacity-80" />
              <div className="absolute w-16 h-8 border-2 border-white border-b-0 bottom-[12%] left-1/2 transform -translate-x-1/2 rounded-t-full opacity-80" />
            </div>
