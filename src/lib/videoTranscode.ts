  ...
  if (!ffmpegPromise) {
    ffmpegPromise = (async () => {
      const [{ FFmpeg }, { toBlobURL }, coreAsset, wasmAsset] = await Promise.all([
        ...
        import('@ffmpeg/util'),
        import('@/assets/ffmpeg/ffmpeg-core-script.asset.json').then((m) => m.default as { url: string }),
        import('@/assets/ffmpeg/ffmpeg-core-binary.asset.json').then((m) => m.default as { url: string }),
      ]);
